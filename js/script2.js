$(function () {
    ...
    // homepage 2,3페이지 중 1번째 이미지 애니메이션
    var animateInHTML = function () {
        var elemsIn,
            windowHeight

        var initIn = function () {
            elemsIn = document.getElementsByClassName("hiddenIn");
            windowHeight = window.innerHeight;
            _addEventHandlersIn();
        }

        var _addEventHandlersIn = function () {
            window.addEventListener("scroll", _checkPositionIn);
            window.addEventListener("resize", initIn)
        }
        var _checkPositionIn = function () {
            for (var r = 0; r < elemsIn.length; r++) {
                var posFromTopIn = elemsIn[r].getBoundingClientRect().top;
                if (posFromTopIn - windowHeight <= 0) {
                    elemsIn[r].className = elemsIn[r].className.replace("hiddenIn", "fadeIn");
                }
            }
        }
    ...
    animateInHTML().initIn();

    // homepage 2,3페이지 중 2번째 이미지 애니메이션
    ...
    // homepage 2,3페이지 중 3번째 이미지 애니메이션
